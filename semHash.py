...
print(np.mean(myData),np.min(myData),np.max(myData))
...
def semHash(data):
    ...
    fc1 = tf.layers.dense(data, h1Size, activation=tf.nn.leaky_relu)
    do1 = tf.nn.dropout(fc1,(1-dORate))

    fc2 = tf.layers.dense(do1, h2Size, activation=tf.nn.leaky_relu)
    do2 = tf.nn.dropout(fc2,(1-dORate))

    fc3 = tf.layers.dense(do2, h2Size, activation=tf.nn.leaky_relu)
    do3 = tf.nn.dropout(fc3,(1-dORate))

    codes = tf.layers.dense(do3, codeSize, activation=tf.nn.leaky_relu)
    do4 = tf.nn.dropout(codes,(1-dORate))

    fc5 = tf.layers.dense(do4, h2Size, activation=tf.nn.leaky_relu)
    do5 = tf.nn.dropout(fc5,(1-dORate))

    fc6 = tf.layers.dense(do5, h2Size, activation=tf.nn.leaky_relu)
    do6 = tf.nn.dropout(fc6,(1-dORate))

   
    fc7 = tf.layers.dense(do6, h1Size, activation=tf.nn.leaky_relu)
